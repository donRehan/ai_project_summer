...
public class Node {
	...
	//test
	public synchronized void createNode(ArrayList<Node> expandedNodes, int[] sizeOfBoard, String moveDirection, block currentObject, int[] miniGoal, block agent) {
		// Clone the current object
		block currentObjectClone = (block) currentObject.clone();
		...
		} else if (currentObject.T.equals("agent")) {
			// Clone the agent object
			block agentClone = (block) agent.clone();

			// Move the agent and create a new node
			currentObjectClone = movement(moveDirection, currentObjectClone);
			Node newNode = createNewNode(sizeOfBoard.clone(), sequence + moveDirection, currentObjectClone, agentClone);
...
private Node createNewNode(int[] sizeOfBoard, String sequence, block currentObject, block agent) {
    Node newNode = next_node(sizeOfBoard, sequence, currentObject, "->change");
    newNode.verticalObjects = copy_arraylist(verticalObjects);
    newNode.horizontalObjects = copy_arraylist(horizontalObjects);
    newNode.agent = agent;
    return newNode;
}
	...
	public synchronized ArrayList expand() {
		...
		if(currentObject.T.equals("agent"))
		{
			...
			if(currentObject.EndPoint[0] <= size_ofBoard[0])
			{
				...
				if(right_spot[0] >= size_ofBoard[0])
				right_spot = null;
				//check if the left spot is occupied
				//Check if it is not occupied then you can move by doing necessary adjustments
			...
			if(currentObject.Direction == 'v')
			{
				...
				if(currentObject.StartPoint[1] > 1)
				{
					...
					else
					{
						...
						// Clone the top_block and assign it to the currentObject
						...
						// Add "AgentChange" to the sequence
						//String seq = sequence + "AgentChange";
